<!-- 四要素认证-开户行名称 -->
<template>
  <div class="nameOfBack">
   <!--  <mt-search v-model="value" autofocus :result.sync="filterResult" placeholder="请输入开户行名称">
   </mt-search> -->
    <div class="mint-search">
      <input type="text" v-model='seachback' placeholder="请输入开户行所在城市名称" @focus="aa = true" @blur="aa = false">
      <img src="/static/images/close.png" v-show='aa && seachback && seachback.length > 0' @click="seachback=''">
    </div>
    <!-- 默认显示的内容 -->
    <ul class="bugs-ul bugs-title">
      <li class="bug-title" style="text-align:left;">
        <span class="bug-title-icon"></span>
        <label class="bug-title-text">银行名称（请选择一条信息）</label>
      </li>
    </ul>
...
export default {
  ...
  computed: {
    filterResult() {
      ...
      return this.newResult = this.newArry.filter(item => new RegExp(this.value, 'i').test(item.bankName));
    }
  },
  methods: {
    ...
    clickBtnItem(item) {
      ...
      console.log(item)
      localStorage.setItem("bankObj", JSON.stringify(item));
