  ...
  methods: {
    getList() {
      this.post('/product/getProductList', {
        modeType: '2', //模式类型 0：极速，1：工薪，2：同业
        pageParam: {
        ...
        if (res.data.code == "100000") {
          ...
          console.log(2, this.productAuthConfigs);
                      ...
                      if (this.newArr.length == 0) {
                        ...
                        for (var i = 0; i < this.newArr.length; i++) {
                          if (this.newArr[i].requestPassId == 1 && this.newArr[i].must == 1 && this.location == 'false') {
                            this.$router.push({ path: '/openPermissions', query: { modeType: '2' } });
                            return;
                          } else {
                            // 存储 productId  , modeType
                            localStorage.setItem('baseObj', JSON.stringify({
    ...
    goRealName(flag) {
      switch (flag) {
        case 'smTipShow':
          localStorage.setItem('jichuOrYewu', 'tongyemoshi');
