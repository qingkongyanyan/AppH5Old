    ...
    <div class="contain">
      ...
      <ul class="feed-back-ul">
        <li class="feed-back-title">
          ...
          <span class="feed-back-del font-color-2DA8E1" v-show="imgUrl1 || imgUrl2" @click="showDelText"><label>{{delStr}}</label></span>
          <!-- 删除 -->
        </li>
      </ul>
      <!-- 上传图片 -->
      <div class="add-img-contain">
        <form class="add-img-form" id="uploadForm1" style="width:100%!important;" v-show="!imgUrl1 && !imgUrl2">
          <input style="display:none" type="file" name="file" accept="image/*" id="fileUpload1" @change="fileUpload($event,'first')" v-if="delStr=='删除'">
          <input style="display:none" type="text" id="fileUpload1" v-if="delStr=='取消'">
          <label class="add-img-normal" for="fileUpload1" v-show="!imgUrl1 && !imgUrl2" style="width:100%!important;">
            <span class="add-img-title font-color-2DA8E1 font-size34">添加图片</span>
          </label>
        </form>
        <!--  -->
        <form class="add-img-form" id="uploadForm1" v-show="imgUrl1||imgUrl2">
          <input style="display:none" type="file" name="file" accept="image/*" id="fileUpload1" @change="fileUpload($event,'first')" v-if="delStr=='删除'">
          <input style="display:none" type="text" id="fileUpload1" v-if="delStr=='取消'">
          <label class="add-img-normal" for="fileUpload1" v-show="!imgUrl1 && !imgUrl2">
            <span class="add-img-title font-color-2DA8E1 font-size34">添加图片</span>
          </label>
          <label class="add-img-item" for="fileUpload1" v-show="!imgUrl1 && imgUrl2">
          </label>
        ...
        <form class="add-img-form" id="uploadForm2">
          <input style="display:none" type="file" name="file" accept="image/*" id="fileUpload2" @change="fileUpload($event,'second')" v-if="delStr=='删除'">
          <input style="display:none" type="text" id="fileUpload2" v-if="delStr=='取消'">
          <label class="add-img-item" for="fileUpload2" v-show="imgUrl1 && !imgUrl2">
          </label>
          <span class="del-icon" v-show="imgUrl2 && delStr=='取消'" @click="delImg('second')"></span>
...
export default {
  ...
  mounted() {
    ...
    this.userName = localStorage.getItem('custName') != 'undefined' ? localStorage.getItem('custName') : '';
