...
<template>
  ...
  <div class="setPassword">
    <!-- 表单 -->
    <form class="form">
      <div class="form-group" :class="{activeb:cc && oldPassword && oldPassword.length > 0}">
        <label>
          <input type="password" placeholder="请输入原密码" v-model="oldPassword" @focus="cc = true" @blur="cc = false" maxlength="16">
        </label>
        <span class="close" v-show='cc && oldPassword && oldPassword.length > 0' @click="oldPassword=''"></span>
      </div>
      <div class="form-group" :class="{activeb:aa && password && password.length > 0}">
        <label>
          <input type="password" placeholder="请输入新密码（包含数字和字母的6-16位密码）" v-model="password" @focus="aa = true" @blur="aa = false" maxlength="16">
        </label>
      ...
      <div class="form-group" :class="{activeb:bb && password2 && password2.length > 0}">
        <label>
          <input type="password" placeholder="请再次输入新密码" v-model="password2" @focus="bb = true" @blur="bb = false" maxlength="16">
        </label>
...
export default {
  ...
  methods: {
    signUp() {
      if (!this.oldPassword) {
        var sel = this;
        this.alentText = '原密码不能为空';
        this.tipShow = true;
        setTimeout(function() { sel.tipShow = false; }, 3000);
        return;
      } else if (this.oldPassword) {
        if (!(/^(?![0-9]+$)(?![a-zA-Z]+$)[0-9A-Za-z]{6,16}$/.test(this.oldPassword))) {
          var sel = this;
          this.alentText = '旧密码格式不正确';
          this.tipShow = true;
          setTimeout(function() { sel.tipShow = false; }, 3000);
          return;
        };
      ...
      this.post('/account/updatePassword', {
        ...
        operType: '1', //操作类型  0-设置密码，1-修改密码
        mobile: this.mobile, //手机号 
        oldPassword: md5(this.oldPassword), //旧密码 操作类型为1时必输
