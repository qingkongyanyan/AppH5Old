...
<template>
  ...
  <div class="mobileeCodeLogin">
    ...
    <form class="form">
      ...
      <p class="signBtn" @click="signUp()">登录</p>
      <!-- <p class="signBtn" v-if="sure" @click="signUp()">登录</p> 
      <p class="signBtnDis" v-if="sureDis">登录</p>  -->
      <p class="codeLogin">
        <span>登录即视作您同意<a :href="signSrc" class="pro-color">《诺远普惠注册协议》</a><a :href="shouquanSrc" class="pro-color">《授权委托书》</a></span>
      </p>
    </form>
    <!-- 弹框 -->
...
export default {
  ...
  mounted() {
    ...
    this.reShow = false;
    this.daoShow = false;
    var codeMobile = localStorage.getItem('codeMobile');
    var codeImgcode = localStorage.getItem('codeImgcode');
    var codeCode = localStorage.getItem('codeCode');
    console.log(codeMobile);
    if(codeMobile){
      this.mobile = codeMobile;
    };
    if(codeImgcode){
      this.imgcode = codeImgcode;
    };
    if(codeCode){
      this.code = codeCode;
    };
  ...
  methods: {
    ...
    getMobilCode() {
      ...
      this.post('/account/getMsgCheckCode', {
        ...
        msgCheckType: 1, //短信验证码类型  1-验证码登录
    ...
    signUp() {
      ...
      this.post('/account/custLogin', {
        mobile: this.mobile.replace(/\s+/g,""), //手机号 
        loginType: 1, //登录类型 1-验证码登录
        msgCheckCode: this.code, //短信验证码 登录类型为1时必输
        password: this.password, //密码  登录类型为0时必输
        imgCheckCodeId: this.codeId, //图片验证码id 登录类型为0时必输
        imgCheckCode: this.imgcode, //图片验证码 登录类型为0时必输
      }).then(res => {
        if (res.data.status == true && res.data.code == 100000) {
          ...
          this.token = res.data.data.token;
            ...
            if(res.data.data.sysCust.custName){
              localStorage.setItem('custName', res.data.data.sysCust.custName);
            };
            if(res.data.data.sysCust.certCode){
          ...
          if(res.data.data.sysCust.password == 'N'){
          ...
          }else if(res.data.data.sysCust.password == 'Y'){
            //成功
            this.$router.push("home");
          };
          
          //localStorage.setItem("loginInformation",JSON.stringify(row));
        };
        if (res.data.status == false && res.data.code != '100000') {
          this.imgcode = '';
    ...
    regMobile(){
      ...
      localStorage.setItem('codeMobile',this.mobile);
    ...
    localImgcode(){
      this.bb = false;
      localStorage.setItem('codeImgcode',this.imgcode);
    },
    Clearimgcode(){
      this.imgcode = '';
      localStorage.setItem('codeImgcode',this.imgcode);
    ...
    localCode(){
      this.cc = false;
      localStorage.setItem('codeCode',this.code);
    },
    Clearcode(){
      this.code = '';
      localStorage.setItem('codeCode',this.code);
